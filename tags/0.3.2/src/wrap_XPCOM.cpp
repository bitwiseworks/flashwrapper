/* $Id: wrap_XPCOM.cpp,v 1.18 2004/07/08 14:14:28 bird Exp $
 *
...
#ifdef NPODIN
/* Save and undef debug define */
#ifdef DEBUG
    #define SAVEDDEBUG 1
    #undef DEBUG
#endif
/* the headers */
#include "moz\sdk\plugin\include\npapi.h"
#include "moz\sdk\plugin\include\npupp.h"
...
#include "moz\sdk\xpcom\include\nsISupportsUtils.h"
#include "moz\sdk\xpcom\include\nsIFile.h"
#include "moz\include\xpcom\nsIEnumerator.h"
#include "moz\sdk\xpcom\include\nsCOMPtr.h"

#include "moz\sdk\xpcom\include\nsIServiceManager.h"
#include "moz\include\xpcom\nsIServiceManagerObsolete.h"
#include "moz\include\xpcom\nsIComponentManagerObsolete.h"
...
#include "moz\include\plugin\nsIPluginTagInfo2.h"
#include "moz\include\plugin\nsICookieStorage.h"
#include "moz\include\plugin\nsIHTTPHeaderListener.h"

#include "moz\include\oji\nsIJRIPlugin.h"
#include "moz\include\oji\nsIJVMPluginInstance.h"
#include "moz\include\oji\nsIJVMManager.h"
#include "moz\include\oji\nsIJVMWindow.h"
#include "moz\include\oji\nsIJVMConsole.h"
#include "moz\include\xpcom\nsISerializable.h"
#include "moz\include\caps\nsIPrincipal.h"
#include "moz\include\oji\nsIJVMPlugin.h"
#include "moz\include\oji\nsIJVMPluginTagInfo.h"
#include "moz\include\oji\nsIJVMPrefsWindow.h"
#include "moz\include\oji\nsILiveConnectManager.h"
#include "moz\include\liveconnect\nsISecurityContext.h"
#include "moz\include\liveconnect\nsILiveConnect.h"
...
#include "moz\sdk\necko\include\nsIRequest.h"
#include "moz\sdk\necko\include\nsILoadGroup.h"
#include "moz\include\xpcom\nsIInterfaceRequestor.h"

#include "moz\sdk\string\include\nsStringDefines.h"
#include "moz\sdk\string\include\nsStringFwd.h"
#include "moz\sdk\string\include\nsBufferHandle.h"
#include "moz\sdk\string\include\nsStringIteratorUtils.h"
#include "moz\sdk\string\include\nsCharTraits.h"
#include "moz\sdk\string\include\nsStringFragment.h"
#include "moz\sdk\string\include\nsCharTraits.h"
#include "moz\sdk\string\include\nsStringTraits.h"
...
#include "moz\sdk\string\include\nsStringIterator.h"
#include "moz\sdk\string\include\nsAString.h"
#include "moz\include\dom\domstubs.h"
#include "moz\include\dom\nsIDOM3Node.h"
#include "moz\include\dom\nsIDOMNode.h"
#include "moz\include\dom\nsIDOMElement.h"


/* restore debug define */
#ifdef SAVEDDEBUG
    #define DEBUG
#endif
...
#ifdef NPODIN
/** Enter the Odin context in a UP worker. */
#define UP_ENTER_ODIN() \
    DEBUG_STACK_ENTER();                                                        \
    unsigned ExceptionRegRec[2] = {0,0};                                        \
    VERIFY_EXCEPTION_CHAIN();                                                   \
    USHORT selFSOld = pfnODIN_ThreadEnterOdinContext(&ExceptionRegRec[0], FALSE); \
    SAVELOAD_FPU_CW(0x37f);                                                     \
    VERIFY_EXCEPTION_CHAIN()

/** Default UP enter code without entering odin context. */
#define UP_ENTER_NOT_ODIN(fGDB) \
    DEBUG_FUNCTIONNAME();                          \
    VERIFY_EXCEPTION_CHAIN();                      \
    UP_TRACE_ENTER();                              \
    DEBUG_GLOBAL_BREAKPOINT(fGDB)

/** Default UP enter code without entering odin context, with default rc declaration. */
#define UP_ENTER_NOT_ODIN_RC(fGDB) \
    DEBUG_FUNCTIONNAME();                          \
    VERIFY_EXCEPTION_CHAIN();                      \
    UP_TRACE_ENTER();                              \
    nsresult    rc = NS_ERROR_UNEXPECTED;          \
...
#ifdef DEBUG
#define UP_LEAVE_EXCPT() \
    ExceptionRegRec[0] = 0xdeadbeef; \
    ExceptionRegRec[1] = 0x11c0ffe1
#else
#define UP_LEAVE_EXCPT() ExceptionRegRec[0] = ExceptionRegRec[1] = 0
#endif

/** Leave Odin context but not the function. */
#define UP_LEAVE_ODIN() \
    VERIFY_EXCEPTION_CHAIN();                                      \
    RESTORE_FPU_CW();                                              \
...
/** Leave a UP working having previously don UP_LEAVE_ODIN(). */
#define UP_LEAVE_NOT_ODIN() \
     UP_TRACE_LEAVE();                                              \
     DEBUG_GLOBAL_BREAKPOINT(1);                                    \
     DEBUG_STACK_LEAVE()

/** Leave a UP working having previously don UP_LEAVE_ODIN(). */
#define UP_LEAVE_NOT_ODIN_INT(rc) \
...
#ifdef NPODIN
/** @name Function Pointers to the Win32 PlugIn
 * @{
 */
static  nsresult (*NPW32CALL pfnNSGetFactory)(/*nsISupports* */ PDOWNTHIS aServMgr,
                                              const nsCID &aClass,
                                              const char *aClassName,
                                              const char *aContractID,
                                              nsIFactory **aFactory);
static PRBool    (*NPW32CALL pfnNSCanUnload)(/*nsISupports* */ PDOWNTHIS aServMgr);
static nsresult  (*NPW32CALL pfnNSRegisterSelf)(/*nsISupports* */ PDOWNTHIS aServMgr, const char *fullpath);
static nsresult  (*NPW32CALL pfnNSUnregisterSelf)(/*nsISupports* */ PDOWNTHIS aServMgr, const char *fullpath);
//@}
#endif
...
public:
    ...
    NS_IMETHOD CreateSecureEnv(JNIEnv* proxyEnv, nsISecureEnv* *outSecureEnv)
    {
        ...
        {
            #ifdef NPODIN
            odinJ2PluginHacks(1, TRUE);     /* #832: focus deadlock hack on */
            #endif
            rc = VFTCALL2((VFTnsIJVMPlugin*)mpvVFTable, CreateSecureEnv, mpvThis, pdownProxyEnv, outSecureEnv);
            #ifdef NPODIN
            odinJ2PluginHacks(1, FALSE);    /* #832: focus deadlock hack off */
            #endif
...
public:
                               ...
                               /*[out]*/ jint* result)
    {
        UP_ENTER_RC();
#ifdef NPODIN
        //@todo Need calling convention wrapping!
        ReleaseInt3(0xbaddbeef, 0x000c0ffe, 0x000c0ffe);
#endif
                         ...
                         /*[out]*/ jint* result)
    {
        UP_ENTER_RC();
#ifdef NPODIN
        //@todo Need wrapping of JavaVM!
        ReleaseInt3(0xbadc00fe, 10, 0x000c0ffe);
#endif
...
public:
    ...
    NS_IMETHOD Destroy(void)
    {
        UP_ENTER_RC();
        #ifdef NPODIN
        odinJ2PluginHacks(1, TRUE);     /* #540: focus deadlock hack on */
        #endif
        rc = VFTCALL0((VFTnsIPluginInstance*)mpvVFTable, Destroy, mpvThis);
        #ifdef NPODIN
        odinJ2PluginHacks(1, FALSE);    /* #540: focus deadlock hack off */
        #endif
        UP_LEAVE_INT(rc);
        return rc;
    }
    ...
    NS_IMETHOD SetWindow(nsPluginWindow * aWindow)
    {
         ...
         */
        /** @todo per instance NPWINDATA like NS4x does. */
        nsPluginWindow *pWindow;
        NPSAVEDPS      PerSetWindow;
        pWindow = (nsPluginWindow *)npWinSetWindowBegin(aWindow, FALSE, NULL, &PerSetWindow, NULLHANDLE, NULLHANDLE);

        /*
         * Now enter Odin context and call the real method.
         */
        UP_ENTER_ODIN();
        rc = VFTCALL1((VFTnsIPluginInstance*)mpvVFTable, SetWindow, mpvThis, pWindow);
        UP_LEAVE_ODIN();
         ...
         */
        /** @todo per instance NPWINDATA like NS4x does. */
        npWinSetWindowEnd(aWindow, FALSE, NULL, &PerSetWindow);
...
//DebugInt3();
         ...
         */
        if (    aPlatformPrint->mode == nsPluginMode_Full
            &&  aPlatformPrint->print.fullPrint.platformPrint == NULL)
        {   /* assumes struct is 100% idendical on windows. */
            dprintf(("%s: *simple printing case*", pszFunction));
            UP_ENTER_ODIN();
        ...
        else
        {
             ...
             * (Must be called out of Odin context!)
             */
            PNPPRINTDATA pPerPrint = npWinPrintBegin(aPlatformPrint, FALSE);
            if (pPerPrint)
            {
                nsPluginPrint *pPrint = (nsPluginPrint *)pPerPrint->pvOdinPrt;

                /*
                 * Now enter Odin context and call the real method.
                 */
                UP_ENTER_ODIN();
                rc = VFTCALL1((VFTnsIPluginInstance*)mpvVFTable, Print, mpvThis, pPrint);
                UP_LEAVE_ODIN();
...
#ifdef NPODIN
DebugInt3(); //curious when/if this ever happens.
#endif
...
public:
    ...
    NS_IMETHOD HandleEvent(nsPluginEvent *aEvent, PRBool *aHandled)
    {
        ...
        nsPluginEvent * pEvent = aEvent;
        if (VALID_PTR(aEvent))
        {
            dprintf(("%s: event=%x wParam=%x lParam=%x (os2)", pszFunction, aEvent->event, aEvent->wParam, aEvent->lParam));
            w32evt.event = aEvent->event;
            w32evt.wParam = aEvent->wParam;
            w32evt.lParam = aEvent->lParam;
            pEvent = (nsPluginEvent*)&w32evt;

            //@todo: switch and convert the actualy messages from OS/2 to Odin world.
            dprintf(("%s: event=%x wParam=%x lParam=%x (odin)", pszFunction, w32evt.event, w32evt.wParam, w32evt.lParam));
            DebugInt3();
        }
#else
    ...
    {
#ifdef NPODIN
        //@todo figure out features on demand, pray we won't need it.
        dprintf(("%s: Incomplete!!!", XPFUNCTION));
#endif
    }
};
...
int upCreateJNIEnvWrapper(JNIEnv **ppJNIEnv, int rc)
{
    ...
    if (VALID_PTR(ppJNIEnv))
    {
        if (VALID_PTR(*ppJNIEnv))
        {
            ...
            {
#ifdef NPODIN
                *ppJNIEnv = (JNIEnv *)NPJNICreateUpWrapper(0, NPJS_TYPE_JNIENV, *ppJNIEnv);
                if (*ppJNIEnv)
                {
#endif
                    /*
                ...
                else
                {
                    ...
                    dprintf(("%s: Failed to make JNIEnv wrapper!!!", pszFunction));
                }
#endif
            }
            else
...
nsresult VFTCALL downIPM2FindProxyForURL(void *pvThis, const char *aURL, char **aResult)
{
    ...
    if (NS_SUCCEEDED(rc) && VALID_PTR(aResult) && VALID_PTR(*aResult))
    #ifdef NPODIN
        dprintf(("%s: *aResult=%s", pszFunction, *aResult));
    #else
    {
        DPRINTF_STR(*aResult);
...
{
    DOWN_ENTER_RC(pvThis, nsIThreadManager);
...
MAKE_SAFE_VFT(VFTnsIThreadManager, downVFTnsIThreadManager)
{
 {
    VFTFIRST_VAL()
    downQueryInterface,                                     VFTDELTA_VAL()
...
}   aDownInterfaces[] =
{
    ...
    { &kThreadManagerIID,           &downVFTnsIThreadManager },
...
int downCreateJNIEnvWrapper(JNIEnv **ppJNIEnv, int rc)
{
    ...
    if (VALID_PTR(ppJNIEnv))
    {
        if (VALID_PTR(*ppJNIEnv))
        {
            ...
            {
#ifdef NPODIN
                *ppJNIEnv = (JNIEnv *)NPJNICreateDownWrapper(0, NPJS_TYPE_JNIENV, *ppJNIEnv);
                if (*ppJNIEnv)
                {
#endif
                    /*
...
void    verifyAndFixUTF8String(const char *pszString, const char *pszFunction)
{
     ...
     * aren't valid UTF8.
     */
    char *pszString2 = *(char**)((void*)&pszString);
    for (char *pch = pszString2; *pch; pch++)
    {
        unsigned ch = *pch;
        if (ch > 0x7f)
        {
        ...
        invalid:
            *pch = ' ';
            dprintf(("%s: non UTF-8 character in string at position %d, char val %d.", pszFunction, pch - pszString2, ch));
        }
    }
    #endif
                               ...
                               const char *aContractID, nsIFactory **aFactory, PNPODINWRAPPER pPlugin)
{
    ...
    {
        dprintf(("%s: npGenericLazyInit() failed returning NS_ERROR_NOT_AVAILABLE", pszFunction));
    ...
    if (!aFactory)
    {
        dprintf(("%s: leave NS_ERROR_NULL_POINTER", XPFUNCTION));
     ...
     */
    if (!pPlugin->pfnW32NSGetFactory)
    {
        dprintf(("%s: pfnW32NSGetFactory is NULL returning NS_ERROR_NOT_IMPLEMENTED", pszFunction));
        return NS_ERROR_NOT_IMPLEMENTED;
    }

    /*
     * Create 'DOWN' wrapper for aServMgr and call the real entry point.
     * There shouldn't be more than one of these...
     */
    PDOWNTHIS pDownServMgr = (PDOWNTHIS)aServMgr;
    nsresult rc = downCreateWrapper((void**)&pDownServMgr, kSupportsIID, NS_OK);
    if (NS_SUCCEEDED(rc))
    {
        /*
         * Call Win32 entrypoint.
         */
        unsigned ExceptionRegRec[2] = {0,0};
        USHORT selFSOld = pfnODIN_ThreadEnterOdinContext(&ExceptionRegRec[0], TRUE);
        rc = pPlugin->pfnW32NSGetFactory(pDownServMgr, aClass, aClassName, aContractID, aFactory);
        pfnODIN_ThreadLeaveOdinContext(&ExceptionRegRec[0], selFSOld);
        ExceptionRegRec[0] = ExceptionRegRec[1] = 0;
        if (NS_SUCCEEDED(rc))
        {
            dprintf(("%s: pfnNSGetFactory succeeded!", XPFUNCTION));
            /*
             * Create 'UP' wrapper.
             * @todo   Create the correct wrapper.
             */
            rc = upCreateWrapper((void**)aFactory, kPluginIID, rc);
        }
        else
        {
            dprintf(("%s: pfnNSGetFactory failed with rc=%x", XPFUNCTION, rc));
            DebugInt3();
        }
    }
    else
        dprintf(("%s: Failed to wrap aServMgr!", pszFunction));


    dprintf(("%s: leave rc=%x", XPFUNCTION, rc));
    return rc;
}
...
DebugInt3();
     ...
     * Is the entrypoint present?
     */
    if (!pPlugin->pfnW32NSCanUnload)
    {
        dprintf(("%s: pfnW32NSCanUnload is NULL returning %d", pszFunction, fRc));
        return fRc;
    }
    ...
    if (NS_SUCCEEDED(rc))
    {
        ...
        unsigned ExceptionRegRec[2] = {0,0};
        USHORT selFSOld = pfnODIN_ThreadEnterOdinContext(&ExceptionRegRec[0], TRUE);
        fRc = pPlugin->pfnW32NSCanUnload(pDownServMgr);
        pfnODIN_ThreadLeaveOdinContext(&ExceptionRegRec[0], selFSOld);
        ExceptionRegRec[0] = ExceptionRegRec[1] = 0;
    }
    else
        dprintf(("%s: failed to create wrapper for nsISupports %p", pszFunction, aServMgr));

    dprintf(("%s: leave fRc=%x", pszFunction, fRc));
    return fRc;
}
...
{
    //@todo TEXT: fullpath? Yes. But haven't seen any use of this entry.
    DEBUG_FUNCTIONNAME();
...
DebugInt3();
     ...
     * Is the entrypoint present?
     */
    if (!pPlugin->pfnW32NSRegisterSelf)
    {
        dprintf(("%s: pfnW32NSRegisterSelf is NULL returning NS_ERROR_NOT_IMPLEMENTED", pszFunction));
        return NS_ERROR_NOT_IMPLEMENTED;
    }
    ...
    if (NS_SUCCEEDED(rc))
    {
        ...
        unsigned ExceptionRegRec[2] = {0,0};
        USHORT selFSOld = pfnODIN_ThreadEnterOdinContext(&ExceptionRegRec[0], TRUE);
        rc = pPlugin->pfnW32NSRegisterSelf(pDownServMgr, fullpath);
        pfnODIN_ThreadLeaveOdinContext(&ExceptionRegRec[0], selFSOld);
        ExceptionRegRec[0] = ExceptionRegRec[1] = 0;
    }
    else
    ...
    dprintf(("%s: leave rc=%x", pszFunction, rc));
    return rc;
}
...
DebugInt3();
     ...
     * Is the entrypoint present?
     */
    if (!pPlugin->pfnW32NSUnregisterSelf)
    {
        dprintf(("%s: pfnW32NSUnregisterSelf is NULL returning NS_ERROR_NOT_IMPLEMENTED", pszFunction));
        return NS_ERROR_NOT_IMPLEMENTED;
    }
    ...
    if (NS_SUCCEEDED(rc))
    {
        ...
        unsigned ExceptionRegRec[2] = {0,0};
        USHORT selFSOld = pfnODIN_ThreadEnterOdinContext(&ExceptionRegRec[0], TRUE);
        rc = pPlugin->pfnW32NSUnregisterSelf(pDownServMgr, fullpath);
        pfnODIN_ThreadLeaveOdinContext(&ExceptionRegRec[0], selFSOld);
        ExceptionRegRec[0] = ExceptionRegRec[1] = 0;
    }
    else
